...
describe("NFTMarket", () => {
  ...
  describe("List", () => {
    it("Should list NFTs", async () => {
      const { MyNFT, NFTMarket, owner, otherAccount } = await loadFixture(
        deployAll
      );
      ...
      const nft = await NFTMarket.read.getListing([0n]);
    ...
    it("Should buy NFTs", async () => {
      ...
      await NFTMarket.write.buyNFT([0n], {
        account: owner.account,
      });

      const boughtAddress = await MyNFT.read.ownerOf([0n]);
      expect(boughtAddress.toLocaleLowerCase()).to.be.equal(owner.account.address); 
    });
  });
});
