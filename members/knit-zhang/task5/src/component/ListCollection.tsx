...
export default function ListCollection({onAdd, isProcessing, setIsProcessing}: {onAdd: (nft: NFT) => void, isProcessing: boolean, setIsProcessing: React.Dispatch<React.SetStateAction<boolean>>}) {
    ...
    const listItem = async(e: React.FormEvent<HTMLFormElement>) => {
        ...
        try{
            ...
            const approveNFT = await TECContract.approvePrice(MarketContract.getAddress(), tokenId);
            await approveNFT.wait();
            const listNFT = await MarketContract.listCollection(TECContract.getAddress(), tokenId, formattedPrice);
            await listNFT.wait();
