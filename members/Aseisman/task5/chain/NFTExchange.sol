...
contract NFTExchange is ReentrancyGuard{
    ...
    //上架授予的NFT
    function ListingNFT(address nftContract, uint256 tokenId, uint256 price, string memory tokenUrl)external {
    ...
    // 下架
    function UnlistingNFT(address nftContract, uint256 tokenId)external {
        Listing storage listing=_Listings[nftContract][tokenId];
        IERC721 nft=IERC721(nftContract);
        
        require(listing.isActive, "this NFT is not listed!");
        
        require(listing.seller==msg.sender, "ur not the seller of this NFT!");
        require(nft.isApprovedForAll(msg.sender, address(this)), "Contract isn't approved.");
        
        _Listings[nftContract][tokenId].isActive=false;
