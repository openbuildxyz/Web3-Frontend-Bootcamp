...
var argv = require("yargs")
  ...
  .default("contract", "0x3E39b9379Db37a15ed63e6eAD7E5d1f468Ae41d2")
  .default("abi", "./build/contract/Counter.abi")
  .default("method", "increment").argv;
...
async function send() {
  ...
  const configJson = JSON.parse(
    fs.readFileSync("./project.config.json", "utf-8").toString()
  );
  // init connection to Artela node
  let node = argv.node ? String(argv.node) : configJson.node;
  if (!node) {
    console.log(
      "'node' cannot be empty, please set by the parameter or artela.config.json"
    );
    process.exit(0);
  }
  ...
  if (!fs.existsSync(senderPriKey)) {
    console.log(
      "'account' cannot be empty, please set by the parameter ' --skfile ./build/privateKey.txt'"
    );
    process.exit(0);
  }
  ...
  if (!contractAddr) {
    console.log(
      "'contract address' cannot be empty, please set by the parameter ' --contract 0x9999999999999999999999999999999999999999'"
    );
    process.exit(0);
  }
  ...
  if (abiPath && abiPath !== "undefined") {
    ...
    console.log(
      "'abi' cannot be empty, please set by the parameter' --abi xxx/xxx.abi'"
    );
    process.exit(0);
  }
  ...
  if (!method || method === "undefined") {
    console.log(
      "'method' cannot be empty, please set by the parameter ' --method {method-name}'"
    );
    process.exit(0);
  }
  ...
  for (let i = 0; i < 100; i++) {
    ...
    try {
      ...
      console.log(
        `transaction throttled, let's wait for 10 seconds: ${e.message}`
      );
