...
contract NFTMarketplace is ReentrancyGuard {
    ...
    event ItemListed(address indexed seller, address indexed nftToken, uint256 tokenId, uint256 price);
    event ItemPurchased(address indexed buyer, address indexed nftToken, uint256 tokenId, uint256 price);
    ...
    function listItem(IERC721 nft, uint256 tokenId, uint256 price) external nonReentrant {
        require(price > 0, "Price must be greater than zero");
        require(nft.getApproved(tokenId) == address(this) || nft.isApprovedForAll(msg.sender, address(this)), "Marketplace must be approved to transfer the item");
    ...
    function purchaseItem(IERC721 nft, uint256 tokenId) external nonReentrant {
        ...
        require(msg.sender != listing.seller, "Seller cannot buy their own listed NFT"); // 增加的检查
