...
contract NFTMarket is Ownable, IERC721Receiver{
    ...
    event NFTlisted(uint256 listingId, address indexed seller, address indexed nftContract, uint256 tokenId, uint256 price);
    event NFTBuy(uint256 listingId, address seller);

    function ListingNft(address nftContract,uint256 price,uint256 tokenId) external {
        // price is not allowed empty
        require(price > 0,"Price must greather than zero");

        totalListings++;
        listings[totalListings] = Listing(msg.sender,nftContract,price,tokenId);
    ...
    function buyListingNft(uint256 listingId) external{
        ...
        _safeTransferForm(paymentToken, msg.sender,listing.seller,listing.price);
    ...
    function _safeTransferForm(IERC20 myToken,address sender,address recipient, uint256 price) private {
