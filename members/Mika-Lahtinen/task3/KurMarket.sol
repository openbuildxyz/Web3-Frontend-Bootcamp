...
contract KurMarket is ReentrancyGuard {
  event NftListed(
  ...
  function sell(address nftContract, uint256 tokenId, uint256 price, string memory tokenUrl) external {
    ...
    if (nftListed) {
      ...
      _listedNfts[nftContract][tokenId] = NftItem(seller, nftContract, tokenId, tokenUrl, price, block.timestamp, true);
