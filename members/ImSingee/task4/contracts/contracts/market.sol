...
contract BryanMarket {
     ...
     function getAllListings() external view returns (Listing[] memory) {
          Listing[] memory allListings = new Listing[](all.length);

          ListingKey memory key;
          for (uint i = 0; i < all.length; i++) {
               key = all[i];
               allListings[i] = listings[key.nftContract][key.tokenId];
          }
     ...
     function listingNFT(address _nft, uint tokenId, uint pricing) external {
          ...
          all.push(ListingKey(_nft, tokenId));
