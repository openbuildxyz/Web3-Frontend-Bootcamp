...
const NftItem= function({
    ...
    if(infoResult.status==="success"){
        const {owner, price} = infoResult.data as {owner:string, price:number};
        _owner = owner;
    ...
    return (
        <div className={styles.nftItem} >
            ...
            <span>售价:{formatUnits(BigInt(_price), 6)}</span>
            {!isOwner&& <Button onClick={() =>{purchase(address, tokenId,_price);}}>购买</Button>}
        </div>
    );
}
