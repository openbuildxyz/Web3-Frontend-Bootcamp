...
class Aspect implements IPreContractCallJP {
    ...
    preContractCall(input: PreContractCallInput): void {
        ...
        const contractAddr = uint8ArrayToHex(input.call!.to);
        const from = uint8ArrayToHex(input.call!.from);
        const storagePrefix = `${contractAddr}:${from}`;
        ...
        const lastExecState = sys.aspect.mutableState.get<u64>(storagePrefix + 'lastExec');
